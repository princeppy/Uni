...
namespace BookShop.Service.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> CreateUser(CreateUserBindingModel createUserModel)
        {
            ...
            var userToAddRolesTo = manager.FindByNameAsync(user.UserName).Result;

            if (createUserModel.RoleName == null)
            {
                manager.AddToRoles(userToAddRolesTo.Id, new string[] { "User" });
            }

            //Uri locationHeader = new Uri(Url.Link("GetUserById", new { id = user.Id }));

            //http://bitoftech.net/2015/01/21/asp-net-identity-2-with-asp-net-web-api-2-accounts-management/
            return Created("", user.UserName /*TheModelFactory.Create(user)*/);
        }
